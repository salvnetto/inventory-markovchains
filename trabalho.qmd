...
Podemos observar através da topologia da cadeia em que não temos estados absorventes, ou seja, não há nenhum estado em que após chegar nele nunca mais sai, e isso faz sentido pois no nosso porbelma estamos lidando com estoques de loja, em nenhum momento o estoque de determinado porduto vai ficar naquela mesma quantidade para sempre, pois temos as demandas, etc. Conseguimos ver também que temos uma classe não essencial que é referente ao estado -1 da cadeia, isso se deve ao fato de que ao atingirmos o limite inferior do estoque( definido como s = 2), nós repomos esse estoque até seu valor máximo ( definido como S = 5), então a partir do momento em que chegamos numa quantidade de estoque inferior a "s" repomos ele até "S". Os outros estados que estão abaixo do limite inferior ou o próprio não são considerados não essenciais pois, a nossa variável $Z_n$(a demanda do n-ésimo dia) foi definida como tendo as seguintes probabilidades:
$P(Z_1 = 0) = 0.3$
$P(Z_1 = 1) = 0.4$
$P(Z_1 = 2) = 0.2$
$P(Z_1 = 3) = 0.1$
...
```
Observando o período da cadeia com a classe não essencial através do comando **period()** ele nós retorna o valor 0, isso se deve ao fato de que como existe uma calsse não essencial e por isso a cadeia é não irredutívele não existe probabilidade de retirno do estado -1 pra ele mesmo,por isso o peírodo é 0.
Agora quando nós retiramos a classe não essencial da cadeia e ficamos apenas com as essencias temos que o período da cadeia é 1, isso se deve ao fato de que $d_i = mdc[n \geq p_{ii} \gt 0]$, o mínimo divisor comum de 1,2,3...
...
Por meio do comando **markovchainSequence()**, nós conseguimos fazer previões de qual a quantidade de estoques que nos temos em "n" passos, no nosso caso quisemos ver qual a quantidade de produtos que teremos em 100 dias começando com o estoque negativo, ao final de 100 dias nos estamos com 2 produtos no estoque. 

```{r}
tabela = table(previsao)
df2 = as.data.frame(tabela)
kable(df2, booktabs = T, linesep = "") %>% 
  kable_styling(latex_options = c("stripped", "HOLD_position", "scale_down")) %>% 
  row_spec(0, bold = T) %>% 
  row_spec(1, bold = T, background  = "lightgray") %>% 
  row_spec(3, bold = T, background  = "lightgray") %>% 
  row_spec(5, bold = T, background  = "lightgray")
```

Essa tabela mostra a frequência com que cada quantidade de pordutos no estoque nos tivemos ao decorrer dos 100 dias, e podemos ver que as quantidades mais frequentes são 2, 3 e 4. 
